...
//    }
	...
	public ResponseEntity<String> depositAmount(@PathVariable int acctID, @PathVariable int amount) {
		...
		double initBal = bankAccountService.getBalance(acctID);
	...
	public ResponseEntity<String> transferAmount(@PathVariable int acctID, @PathVariable int destAcctID,
		...
		Transaction transactionSender = new Transaction(acctID, "Transferred", "Success", initBalSender,
		...
		return ResponseEntity.status(HttpStatus.OK)
				.body("Amount " + amount + "Transfered from " + acctID + " to " + destAcctID);
	...
	public ResponseEntity<String> transferCurrency(@PathVariable int acctID, @PathVariable int destAcctID,
		...
		Transaction transactionSender = new Transaction(acctID, "Transferred", "Success", initBalSender,
				initBalSender - currencyConverted.getAmount(), currencyConverted, sender, receiver);

		transactionSender.setTransaction_date(LocalDate.now());

		transactionSender.setTransaction_time(LocalTime.now());
		...
		loggerReceiver.setTransaction_date(LocalDate.now());

		loggerReceiver.setTransaction_time(LocalTime.now());

		transactionService.addLog(loggerReceiver);

		return ResponseEntity.status(HttpStatus.OK)
				.body("Amount " + currencyConverted.getAmount() + "Transfered from " + acctID + " to " + destAcctID);
